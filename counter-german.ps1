...
while ($True) {
    ...
    $cpu_rate_pss = (Get-Counter "\Prozess($ps_name*)\Prozessorzeit (%)").CounterSamples | Sort-Object Path

    #$cpu_rate_sys = (Get-Counter "\processor(_total)\% processor time").CounterSamples | Sort-Object Path
    $cpu_rate_sys = (Get-Counter "\Prozess(*)\Prozessorzeit (%)").CounterSamples | Sort-Object Path
    ...
    $memory_pss   = (Get-Counter "\Prozess(*)\Arbeitsseiten - privat").CounterSamples | Sort-Object Path
